...
const initiatePayment = async (payload: {
  ...
  try {
    ...
    if (isSuccess) {
      ...
      booking.driverId = new mongoose.Types.ObjectId(payload.driverId);
      booking.status = "confirmed";
      booking.paymentStatus = "paid";
      booking.payment = payment[0]._id;

      booking.driverQuote.forEach((quote) => {
        if (quote.driverId.toString() === payload.driverId.toString()) {
          quote.status = "confirmed" as any; // temporary cast to bypass TS
        }
      });
